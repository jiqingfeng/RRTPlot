...
plotter.add_obstacle(obs1, color='#34495e')
plotter.add_obstacle(np.array([[5, 1], [7, 1], [6, 3]]), color='#7f8c8d')
...
for idx, (x, y, r) in enumerate(nodes):
    ...
    plotter.add_circle((x, y), radius=r, color=color, label=f'q{idx}')

# 连接节点（模拟RRT扩展路径）
plotter.connect_circles(nodes[0], nodes[1], label='Step 1', color='#2980b9')
plotter.connect_circles(nodes[1], nodes[2], label='Step 2', color='#2980b9')
plotter.connect_circles( nodes[3], nodes[4], label='Step 4', color='#2980b9')
